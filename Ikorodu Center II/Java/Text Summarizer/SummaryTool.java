...
class SummaryTool{
	...
	void init(){
		...
		noOfParagraphs = 0;
		try {
	        in = new FileInputStream("C:\\Users\\HP\\Videos\\text-summarizer-master\\src\\text\\summarizer\\master\\article\\article");
	...
	void createSummary(){
	      ...
	      for(int j=0;j<=noOfParagraphs;j++){
	      		int primary_set = paragraphs.get(j).sentences.size()/10; ;
